...
try:
    ...
    # Fallback for environments where keras is accessed via TF
    from tensorflow import keras

# -------------------- Helpers & Config --------------------
APP_TITLE = "KCP Share Prediction Dashboard"
BASE_DIR = Path(__file__).resolve().parent

# Data/model locations are resolved in this order:
# 1) st.secrets["paths"]["MODEL_PATH"] / ["DATA_PATH"]
# 2) ENV vars: KCP_MODEL_PATH / KCP_DATA_PATH
# 3) Repo-relative defaults: ./models/model.keras and ./data/Test.xlsx
def get_path(kind: str) -> Path:
    # kind ∈ {"model", "data"}
    # Try secrets
    try:
        s = st.secrets["paths"][f"{kind.upper()}_PATH"]
        if s:
            p = Path(s)
        ...
        pass
    # Try environment
    env_name = f"KCP_{kind.upper()}_PATH"
    if os.getenv(env_name):
        p = Path(os.getenv(env_name))
        return p if p.is_absolute() else (BASE_DIR / p)
    # Fallbacks
    if kind == "model":
        # Prefer .keras; otherwise .h5; otherwise SavedModel dir
        for candidate in [
            ...
            if candidate.exists():
                return candidate
        # Last resort: original Windows path, but won't exist on Cloud
        return Path(r"C:\Users\W64295\OneDrive - Kimberly-Clark\2025\DS\categoryShareModel.mft.kcp.keras")
    else:
        # Data
        for candidate in [
            BASE_DIR / "data" / "Test.xlsx",
            BASE_DIR / "Test.xlsx",
        ]:
            if candidate.exists():
                return candidate
        return BASE_DIR / "Test.xlsx"
...
unsafe_load = st.sidebar.checkbox("🔓 Enable unsafe deserialization (Lambda layers)", value=False,
                                  help="Only enable if your model fails to load due to Lambda deserialization.")
...
# Versions block – helps diagnose Cloud vs local mismatches
import tensorflow as tf
st.info({
    "Python": sys.version.split()[0],
    "OS": os.name,
    "TF": tf.__version__,
    "Keras": getattr(keras, "__version__", "n/a"),
...
def load_data(path: Path):
    ...
    # Robust Excel engine selection
    try:
        df = pd.read_excel(path, engine="openpyxl")
    except Exception:
        df = pd.read_excel(path)  # let pandas pick an available engine
    ...
    baseline_prices = pd.to_numeric(
        df.loc[price_mask].iloc[0][feature_cols], errors="coerce"
    ).fillna(0.0)
    baseline_prices.index = feature_cols

    share_mask = labels.eq("untsshr") | labels.str.contains("untsshr")
    if share_mask.any():
        shares_all = pd.to_numeric(
            df.loc[share_mask].iloc[0][feature_cols], errors="coerce"
        ).fillna(0.0)
    ...
    # Normalize if shares look like proportions 0..1
...
# -------------------- Model Loading (portable) --------------------
def get_custom_objects():
    """Optional: import/define custom objects to avoid Lambda marshal issues."""
    custom_objects = {}
    # Example custom layer replacement of Lambda; register serializable classes/functions if you have them.
    # from my_module import my_lambda, MyTransform
    # custom_objects["my_lambda"] = my_lambda
    # custom_objects["MyTransform"] = MyTransform
    return custom_objects

@st.cache_resource(show_spinner=False)
def load_model_portable(path: Path, unsafe: bool = False):
    if not path.exists():
        raise FileNotFoundError(f"Model not found: {path}")

    # Prefer compile=False to avoid optimizer/loss deserialization issues across versions
    kwargs = dict(compile=False)

    # Keras v3: safe_mode argument is deprecated; enable unsafe via config if requested
    if unsafe:
        try:
            # Enable unsafe deserialization for Lambda layers etc.
            keras.config.enable_unsafe_deserialization()
        except Exception:
            pass

    # Supply custom objects (if any)
    custom_objects = get_custom_objects()
    if custom_objects:
        kwargs["custom_objects"] = custom_objects

    # Attempt load with multiple strategies
    # 1) Direct load (works for .keras/.h5/SavedModel if compatible)
    try:
        model = keras.models.load_model(path, **kwargs)
        return model
    except Exception as e1:
        # 2) If path is a directory (SavedModel), try that explicitly
        if path.is_dir():
            try:
                model = keras.models.load_model(str(path), **kwargs)
                return model
            except Exception as e2:
                pass
        # 3) Final attempt: if unsafe not enabled, retry with unsafe flag
        if not unsafe:
            try:
                keras.config.enable_unsafe_deserialization()
                model = keras.models.load_model(path, **kwargs)
                ...
                raise RuntimeError(
                    f"Failed to load model. First error: {e1}\n"
                    f"Second error: {e2 if 'e2' in locals() else 'n/a'}\n"
                    f"Unsafe retry error: {e3}"
                )
        else:
...
with st.spinner("📁 Loading..."):
    ...
    model = load_model_portable(MODEL_PATH, unsafe=unsafe_load)
...
UPDATED_INPUTS = {
    c: (float(edited_original.loc["Updated Price", c]) if c in EDITABLE_COLS else BASELINE_INPUTS[c])
    for c in FEATURE_COLS
}
...
# Avoid duplicate model "builds" – just predict
...
def convert_outputs(arr):
    ...
    if arr.size >= N_SKU + 1:
        ...
        # If looks like probabilities, convert to %
...
# If competing share not present, use heuristic for total (your original behavior)
...
with tab_analysis:
    ...
    col1, col2 = st.columns(2)
    n_base = len(baseline_sku_pct)
    n_upd  = len(updated_sku_pct)
